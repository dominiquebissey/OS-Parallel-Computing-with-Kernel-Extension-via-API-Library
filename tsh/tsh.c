...
/*---------------------------------------------------------------------------
  ...
  Date        : April '25
  Coded by    : 
  Modification: Dominique Bissey
---------------------------------------------------------------------------*/

//Read tsh_shell_it to get length (remember to nthol(length))
//read the stream
//parse (lab4)
//Fork and esecute (lab4)
//Build ACK use redirection code in lab4 to capture stdout of subprocess and store it in 
//tsh_shell_ot
//write (tsh_shell_ot)
...
void OpShell() {
   tsh_shell_it in;
    ...
    // Debug: Read input message from client
    printf("OpShell: Waiting for input...\n");

    if (readn(newsock, (char *)&in, sizeof(tsh_shell_it)) == -1) {
        perror("OpShell: readn failed for tsh_shell_it");
        return;
    }

    length = ntohl(in.length);
    printf("OpShell: Command length = %zu\n", length);


    buff = (char *)malloc(length + 1); 
    if (buff == NULL) {
        perror("OpShell: malloc failed for buffer");
        return;
    }

    if (readn(newsock, buff, length) == -1) {
        perror("OpShell: readn failed for command");
        free(buff);
        return;
    }

    buff[length] = '\0'; 
    printf("OpShell: Received command: %s\n", buff);

    parse_input(buff, args);
    printf("OpShell: Command parsed into arguments.\n");

    pid = fork();
    if (pid == -1) {
        perror("OpShell: fork failed");
        free(buff);
        return;
    }

    if (pid == 0) {
         // Debug: Forked child process
        printf("OpShell: Child process for command execution.\n");

        if (execvp(args[0], args) == -1) {
            perror("OpShell: execvp failed");
            exit(EXIT_FAILURE);
        }
    } else {
        waitpid(pid, &status, 0);
        printf("OpShell: Child process finished executing.\n");


        out.status = htons(SUCCESS); 
        out.error = htons(TSH_ER_NOERROR);

        snprintf(out.stdout, sizeof(out.stdout), "Command executed: %s", buff);

        if (writen(newsock, (char *)&out, sizeof(tsh_shell_ot)) == -1) {
            perror("OpShell: writen failed for tsh_shell_ot");
        } else {
            // Debug: Successful write to client
            printf("OpShell: Sent output back to client.\n");
        }
    }
